...
class The_Bootstrap_Gallery_Widget extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct( 'cabarave-gallery', __( 'Cabarave Gallery Widget', 'cabarave' ), array(
			'classname'		=>	'cabarave-gallery',
			'description'	=>	__( 'Displays gallery images of a specified post with the Gallery post format.', 'cabarave' )
		) );
	}
	...
	public function widget( $args, $instance ) {
		...
		if ( $title = get_the_title( $instance['post_id'] ) )
			echo $before_title . '<a href="' . get_permalink( $instance['post_id'] ) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'cabarave' ), strip_tags( $title ) ) . '" rel="bookmark">' . $title . '</a>' . $after_title;
		?>
		<div id="sidebar-gallery-slider" class="carousel slide">
			...
			<!-- Carousel nav -->
			<a class="carousel-control left" href="#sidebar-gallery-slider" data-slide="prev"><?php _ex( '&lsaquo;', 'carousel-control', 'cabarave' ); ?></a>
			<a class="carousel-control right" href="#sidebar-gallery-slider" data-slide="next"><?php _ex( '&rsaquo;', 'carousel-control', 'cabarave' ); ?></a>
	...
	public function form( $instance ) {
		...
		if ( empty( $gallery_posts ) ) {
			echo '<p class="description">'. sprintf( __( 'No galleries have been created yet. <a href="%s">Create some</a>.', 'cabarave' ), admin_url( 'post-new.php' ) ) . '</p>';
			return;
		}
		?>
		<p>
			<label for="<?php echo $this->get_field_id( 'post_id' ); ?>"><?php _e( 'Select Gallery:', 'cabarave' ); ?></label>
...
/* End of file cabarave-gallery-widget.php */
/* Location: ./wp-content/themes/cabarave/cabarave-gallery-widget.php */