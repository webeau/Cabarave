...
function cabarave_comment_nav() {
	...
	<nav class="comment-nav well">
		...
		<div class="nav-previous alignleft"><?php next_comments_link( __( '&larr; Newer Comments', 'cabarave' ) ); ?></div>
		<div class="nav-next alignright"><?php previous_comments_link( __( 'Older Comments &rarr;', 'cabarave' ) ); ?></div>
	</nav>
...
function cabarave_posted_on() {
	printf( __( '<span class="sep">Posted on </span><a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s" pubdate>%4$s</time></a><span class="by-author"> <span class="sep"> by </span> <span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>', 'cabarave' ),
			...
			esc_attr( sprintf( __( 'View all posts by %s', 'cabarave' ), get_the_author() ) ),
			get_the_author()
	);
	if ( comments_open() AND ! post_password_required() ) { ?>
		<span class="sep"> | </span>
		<span class="comments-link">
			<?php comments_popup_link( '<span class="leave-reply">' . __( 'Leave a reply', 'cabarave' ) . '</span>', __( '<strong>1</strong> Reply', 'cabarave' ), __( '<strong>%</strong> Replies', 'cabarave' ) ); ?>
		</span>
		<?php
	}
	edit_post_link( __( 'Edit', 'cabarave' ), '<span class="sep">&nbsp;</span><span class="edit-link label">', '</span>' );
}
endif;
 ...
 *
 * It's basically the wp_link_pages() function, altered to fit to Cabarave
...
function cabarave_link_pages( $args = array() ) {
	...
	$defaults = array(
		...
		'nextpagelink'		=> __('Next page', 'cabarave'),
		'previouspagelink'	=> __('Previous page', 'cabarave'),
		'pagelink'			=> '%',
		'echo'				=> true
	);
	...
	if ( $multipage ) {
		if ( 'number' == $next_or_number ) {
			$output .= '<nav class="pagination clear"><ul><li><span class="dots">' . __('Pages:', 'cabarave') . '</span></li>';
...
/* Location: ./wp-content/themes/cabarave/inc/template-tags.php */