	...
	<div id="content" role="main">
		...
		<nav id="nav-single" class="well clearfix">
			<span class="gallery-link pull-left">
				...
				<?php printf(
					_x( '&laquo; %1$s (%2$s)', 'Post title, amount of images', 'cabarave' ),
					get_the_title( $post->post_parent ),
					sprintf(
						_nx( '%d image', '%d images', $total_images, 'Amount of images', 'cabarave' ),
						$total_images
					)
				); ?>
				</a>
			...
			<span class="nav-links pull-right">
				...
				if ( isset($attachments[$k+1]) )
					echo ' &mdash; <a href="' . get_permalink( $attachments[$k+1]->ID ) . '">' . __( 'Next Photo &raquo;', 'cabarave' ) . '</a>';
				?>
			</span>
		</nav><!-- #nav-single -->
...
/* Location: ./wp-content/themes/cabarave/image.php */