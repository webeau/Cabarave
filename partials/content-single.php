...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<footer class="entry-footer">
		...
		$tags_list = get_the_tag_list( '', _x( ', ', 'used between list items, there is a space after the comma', 'cabarave' ) );
		
		if ( $categories_list )
			printf( '<span class="cat-links block">' . __( 'Posted in %1$s.', 'cabarave' ) . '</span>', $categories_list );
		if ( $tags_list )
			printf( '<span class="tag-links block">' . __( 'Tagged %1$s.', 'cabarave' ) . '</span>', $tags_list );
		?>
	</footer><!-- .entry-footer -->
	...
	<div id="author-description" class="span7">
		...
		<div id="author-link">
			<a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ); ?>" rel="author">
				<?php printf( __( 'View all posts by %s <span class="meta-nav">&rarr;</span>', 'cabarave' ), get_the_author() ); ?>
			</a>
		</div><!-- #author-link	-->
...
/* Location: ./wp-content/themes/cabarave/partials/content-single.php */